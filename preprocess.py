...
def image_to_12_ratio(img):
    h, w, _ = img.shape
    if (2 * h >= w):
        img_12 = img
    else:
        border = int(((w / 2) - h) // 2)
        img_12 = cv2.copyMakeBorder(img, border, border, 0, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
...
if __name__ == '__main__':
    ...
    max_w = 1024
    w_percent = (max_w / float(img_input.size[0]))
    h_size = int((float(img_input.size[1]) * float(w_percent)))
    img_input = img_input.resize((max_w, h_size), Image.Resampling.LANCZOS)
    
    img_ori = np.array(img_input)

    img_raw = image_to_12_ratio(img_ori) 
    
    print(f"Original size: {img_ori.shape}, Padded size: {img_raw.shape}")

    print("Estimating gravity (Z-axis)...")
    olines, vp, views, edges, panoEdge, score, angle = panoEdgeDetection(img_raw)
    ...
    img_rotated = rotatePanorama(img_raw.astype(np.float64) / 255.0, vp[2::-1])
    

    final_img = Image.fromarray((img_rotated * 255).astype(np.uint8))
